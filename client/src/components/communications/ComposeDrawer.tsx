...
interface ComposeDrawerProps {
  ...
  onSuccess: () => void;
  /** Pre-select a tenant when opened from a tenant detail page */
...
export function ComposeDrawer({
  ...
  const applyTemplate = (templateBody: string) => {
    ...
    const filled = tenant
      ? templateBody.replace("[Tenant Name]", tenant.full_name)
      : templateBody;
    setBody(filled);
  };
    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
        <SheetHeader>
          ...
          <SheetDescription>
            Send an email or WhatsApp message to a tenant.
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-5 py-6">
          ...
          <div className="space-y-2">
            ...
            <Select
              ...
              disabled={tenantsLoading || !!preselectedTenantId}
            >
          ...
          {/* WhatsApp link state — shown after logging */}
          {waLink && (
            <div className="rounded-md border border-green-300 bg-green-50 p-4 space-y-2">
              <p className="text-sm font-medium text-green-800">
                Message logged. Open WhatsApp to send it from your device.
              </p>
              <Button
                type="button"
                variant="outline"
                size="sm"
                className="text-green-700 border-green-400 flex items-center gap-1.5"
