...
import { authService, RegisterSchema, LoginSchema, VerifyEmailSchema, ForgotPasswordSchema, ResetPasswordSchema } from '../services/auth.service';
...
export const authController = {
  ...
  async me(req: any, res: Response) {
    try {
      ...
      const user = await prisma.user.findUnique({ where: { id: req.user.sub }, select: { id: true, email: true, full_name: true, role: true } });
      if (!user) { res.status(404).json({ success: false, error: 'User not found.' }); return; }
      ok(res, { data: { id: user.id, email: user.email, name: user.full_name, role: user.role } });
    } catch (e: any) { fail(res, e); }
  },
};
