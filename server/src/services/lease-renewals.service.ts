...
class LeaseRenewalsService {
   ...
   * read an empty column). Does NOT touch the lease document - generating
   * one is a separate, explicit action (TenanciesService.generateLease)
   * the agent triggers whenever they're ready; it always reads the
   * tenancy's current lease_end, so it picks up this renewal automatically
   * whenever it's next generated.
   */
  async create(data: CreateRenewalDto, user: TokenPayload) {
    ...
    const currentLeaseEnd = tenancy.lease_end ?? tenancy.lease_start;
    if (newLeaseEnd <= currentLeaseEnd) throw new AppError('The new lease end date must be after the current lease end date.', 400);
