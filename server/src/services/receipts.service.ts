...
export class ReceiptsService {
  ...
  async generateReceiptPdf(paymentId: string, user: TokenPayload): Promise<Buffer> {
    ...
    return new Promise((resolve, reject) => {
      ...
      // ── AGENCY DETAILS ────────────────────────────────────────────────────
      hr();
      doc.fillColor(BLUE).fontSize(11).font('Helvetica-Bold').text(account?.name ?? 'Property Manager', { align: 'center' });
      ...
      // ── AMOUNT BLOCK ─────────────────────────────────────────────────────
      const amtY = doc.y;
      doc.rect(50, amtY, W, 46).fill('#f0f7ff');
      ...
      doc.fillColor(BLUE).fontSize(8)
        .text(`Generated by PropManager · ${new Date().toLocaleString('en-GB')}`, { align: 'center', width: W });
    ...
    if (channel === 'email') {
      ...
      try {
        ...
        await this.resend.emails.send({
          from: account?.email ? `${account.name} <${account.email}>` : `PropManager <onboarding@resend.dev>`,
          to: [tenant.email],
